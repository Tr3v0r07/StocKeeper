    ...
    <form method="POST" action="{{ route('newUser') }}">
        ...
        <!-- Name -->
            <div >
                ...
                <x-input id="first_name" class="block mt-1 w-80" type="text" name="first_name" value="{{ $user->first_name }}" required autofocus />
            </div>
            ...
            <div class="inline">
                <select class="mt-6 rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" name="role" id="role">
                    <option value="Admin">Admin</option>
                    <option value="User">User</option>
                </select>
            </div>
            ...
            <button class="'inline float-right mt-7 items-center  px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150'">
                {{ __('Confirm') }}
            </button>
